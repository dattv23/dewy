      ...
      <div className="mx-auto flex h-16 w-full max-w-6xl items-center gap-3 px-4">
        ...
        <Button asChild variant="ghost" className="relative h-11 w-11 rounded-lg p-0">
          <Link href="/gio-hang" aria-label="Giỏ hàng">
            ...
            {count > 0 && (
              <span className="bg-primary text-primary-foreground absolute -top-1 -right-1 inline-flex min-h-5 min-w-5 items-center justify-center rounded-full px-1 text-[11px] font-semibold">
                {count}
              </span>
            )}
          </Link>
          ...
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 px-4 py-4">
            ...
            {WEBSITE_NAVIGATION.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="text-foreground rounded-lg px-2 py-2 text-[15px]"
